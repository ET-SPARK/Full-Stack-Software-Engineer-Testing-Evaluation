...
async function main() {
  ...
  const report = {
    ...
    compliance_check: {
      browser_only: testResults.tests.find(t => t.name.includes('Browser-Only Implementation'))?.outcome === 'passed',
      fifo_logic: testResults.tests.find(t => t.name.includes('FIFO'))?.outcome === 'passed',
      no_hardcoded_values: testResults.tests.find(t => t.name.includes('hardcoded'))?.outcome === 'passed',
      performance_optimized: testResults.tests.find(t => t.name.includes('O(n)'))?.outcome === 'passed',
      six_ui_sections: testResults.tests.find(t => t.name.includes('components'))?.outcome === 'passed'
    },
    final_verdict: {
